...
export default function MainStakeholdersSurvey() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // اینجا می‌تونی اطلاعات رو به API هم بفرستی
      console.log("Answers:", answers);

      toast.success("پرسشنامه با موفقیت ارسال شد!");
      setAnswers({});
            ...
            <div key={categoryIndex} className="mb-8">
              ...
              <table className="border-collapse border border-gray-300 w-full mb-4">
                <thead>
                  ...
                  <tr>
                    <th className="border border-gray-300" colSpan={2}></th>
                    {[1, 2, 3, 4, 5].map((score) => (
                      <th
                        key={score}
                    ...
                    return (
                      <tr key={questionId}>
                        ...
                        {[1, 2, 3, 4, 5].map((score) => (
                          <td
                            key={score}
                            className="border border-gray-300 text-center px-2 py-2"
                          >
                            <input
                              type="radio"
                              name={`q${questionId}`}
